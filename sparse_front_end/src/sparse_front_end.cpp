...
SparseFrontEnd::~SparseFrontEnd() {
  ...
  if (relocalizer_thread_.joinable()) {
...
void SparseFrontEnd::reset()
{
  ROS_ERROR("Front end reset NOT IMPLEMENTED!!!");
}

bool SparseFrontEnd::Init(
  ...
  system_status_.rng.seed(CommonFrontEndConfig::getConfig()->getRansacSeed());
  ...
  old_rig_ = rig;
  tracking_stats_.SetRigSize(old_rig_.cameras.size());
...
//  back_end_opt_.Init(map_);
  ...
  if (frames->Size() > 0) {
    unsigned int wide_fov_cam_id = calibu::LargestFrustum(old_rig_);
    dense_aligner_.Init(old_rig_.cameras[wide_fov_cam_id].camera, 32,
  ...
  // Process first frame
  //=========================================================
  tracking_stats_.match_flags_histogram.resize(old_rig_.cameras.size());
  ...
  if (has_imu) {
    CHECK(!front_end_opt_.GetImuBuffer().elements.empty());
  ...
  current_frame_->set_t_vs(old_rig_.cameras[0].T_wc);
  current_frame_->set_cam_params(old_rig_.cameras[0].camera.GenericParams());
  reference_frame_ = current_frame_;

  // Allocate the feature images.
  current_feature_images_.resize(old_rig_.cameras.size());
  keyframe_images_.resize(old_rig_.cameras.size());
  ...
  StartNewLandmarks(old_rig_, map_, current_frame_, current_feature_images_,
                    system_status_.num_tracked_landmarks,
                    system_status_.num_new_landmarks,
                    feature_mask_);
  ...
  //=========================================================
  system_status_.frame_number = 0;
  system_status_.keyframe_number = 0;
  system_status_.time = timestamp;
  system_status_.inlier_noise_error = inlier_threshold_;

  learning_rate_ = 0.9;  // slow
  system_status_.is_initialized =  false;

  system_status_.drop_new_frame = true;
...
bool SparseFrontEnd::IsInitialized() const {
  return system_status_.is_initialized;
}

bool SparseFrontEnd::SwitchToKeyframe(const ReferenceFrameId& closest_keyframe_id,
...
bool SparseFrontEnd::IterateBa() {
  ...
  if (!system_status_.is_imu_converged) {
    depth = system_status_.keyframe_number;
    num_iterations = 4;
  ...
  front_end_opt_.RefineMap(depth,
                          current_frame_->id(),
                          num_iterations,
                          has_imu_,
                          old_rig_,
  ...
  if (!system_status_.is_imu_converged && system_status_.keyframe_number > 20) {
    SlamFramePtr initial = map_->GetFramePtr(initial_frame_);
    CHECK(initial);  // Something's wrong if we don't have our initial pose
    ...
    if (gravity_diff < 0.1 && bias_diff < 0.1 && velocity_diff < 0.1) {
      system_status_.is_imu_converged = true;
    }
  }
  return true;
}
...
void SparseFrontEnd::ProcessKeyframe(
  ...
  //=========================================================
  if (system_status_.tracking_status == common::eTrackingGood) {
...
bool SparseFrontEnd::Iterate(const std::shared_ptr<pb::ImageArray>& frames,
                       double timestamp) {
  ...
  //=========================================================
  if (!system_status_.is_initialized) {
    system_status_.is_initialized = Initialization(frames, timestamp);
    system_status_.tracking_status = common::eTrackingGood;
    Toc("FrontEnd");
    return true;
  }
  ...
  //=========================================================
  Sophus::SE3t t_ab;
  if (system_status_.drop_new_frame) {
    reference_frame_ = current_frame_;
    ...
    current_frame_->set_time(system_status_.time);
  ...
  if (do_dense_init_) {
    ...
    unsigned int cam_id = calibu::LargestFrustum(old_rig_);
  ...
  if (has_imu_ && use_imu_for_gn_) {
    meas = front_end_opt_.GetImuBuffer().GetRange(
        reference_frame_->time(), timestamp);
    use_imu_for_estimate = !meas.empty();

    Eigen::Vector3t g_r_new, v_r_new;
    front_end_opt_.IntegrateLastPose(reference_frame_->time(),
                                    timestamp,
  ...
  tracking_stats_.analytics["MRE"] = system_status_.rmsre;
  tracking_stats_.analytics["InlierNoise"] = system_status_.inlier_noise_error;

  if (failed && use_imu_for_estimate) {
    t_ab = t_ab_imu;
    system_status_.tracking_status = common::eTrackingBad;
  } else if (failed) {
  ...
  if (SlamEdgePtr edge = map_->GetEdgePtr(reference_frame_->id(),
                                          current_frame_->id())) {
    bool is_broken = (system_status_.tracking_status == common::eTrackingBad &&
                      !has_imu_);
  ...
  if (IsKeyframe(t_ab, new_measurements) ||
    ...
    map_->AddNewMeasurementsToFrame(current_frame_->id(), new_measurements);
    system_status_.drop_new_frame = false;
    t_ab_ = t_ab;
 ...
 * ISMAR 2013
 */
bool SparseFrontEnd::Initialization(const std::shared_ptr<pb::ImageArray>& frames,
                              double timestamp) {
  if (old_rig_.cameras.size() > 1) return true;
  ...
  unsigned int n = old_rig_.cameras[0].camera.NumParams();
  Scalar* params = old_rig_.cameras[0].camera.data();
  Scalar w = 0.0;
  if (n == 5) {
    w = std::fabs(params[4]);
  }
  ...
  if (mean_disparity > init_min_disparity_ &&
    ...
    ba_init.Init(options, 1, new_measurements.size(), new_measurements.size(),
                 old_rig_.cameras[0].T_wc);
    for (unsigned int ii = 0; ii < old_rig_.cameras.size(); ++ii) {
      ba_init.AddCamera(old_rig_.cameras[ii].camera, old_rig_.cameras[ii].T_wc);
    }
  ...
  if (landmark_pct > init_min_pctg_init_landmarks_) {
    ...
    StartNewLandmarks(old_rig_, map_, current_frame_, current_feature_images_,
                      system_status_.num_tracked_landmarks,
                      system_status_.num_new_landmarks, feature_mask_);
    Toc("StartNewLandmarks");
    Tic("BA");
    IterateBa();
    Toc("BA");
    system_status_.drop_new_frame = true;
    t_ab_ = Sophus::SE3t();
    if (system_status_.frame_number < init_min_keyframes_) {
...
void SparseFrontEnd::Clear() {
  system_status_.Clear();
  tracking_stats_.analytics.clear();
}

bool SparseFrontEnd::IsKeyframe(
    const Sophus::SE3t& t_ab,
...
void SparseFrontEnd::ProcessFailedTracks(
...
void SparseFrontEnd::PrintTrack(const Landmark &lm) {
...
bool SparseFrontEnd::ExtractFeatures(const std::shared_ptr<pb::ImageArray>& frames,
...
void SparseFrontEnd::ResetFeatures(FeatureImageVector images) {
  for (size_t ii = 0; ii< images.size(); ++ii) {
    images[ii]->ResetFeatures();
  }
}

void SparseFrontEnd::PrintPoseCart(Sophus::SE3t &Tab) {
...
Scalar SparseFrontEnd::ComputeRMSRE(const unsigned int window_size) {
  LocalMap localMap;
  ...
  for (auto & pair : localMap.landmarks) {
    ...
    for (std::shared_ptr<MeasurementContainer> z : container->measurements) {
      ...
      Sophus::SE3t Tsw  = old_rig_.cameras[z->cam_id].T_wc.inverse() *
          localMap.poses[z->id.frame_id].t_wp.inverse();

      const Vector4t x_s = Sophus::MultHomogeneous(Tsw, container->x_w);
      const Vector2t p =
          old_rig_.cameras[z->cam_id].camera.Project(x_s.head<3>());
...
ReferenceFrameId SparseFrontEnd::GetClosestKeyframeId(const ReferenceFrameId& root_id,
...
void SparseFrontEnd::AddPlace(const ReferenceFrameId& id, const cv::Mat& img) {
  //=========================================================
...
void SparseFrontEnd::Save(const std::string& /*filename*/) const {
}

void SparseFrontEnd::Load(const std::string& /*filename*/) {
}
...
void SparseFrontEnd::configCallback(sparse_front_end::SparseFrontEndConfig &config, uint32_t level)
{
