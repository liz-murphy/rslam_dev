...
public:
  ...
  /**
   * Copies from a vector the elements of the given indices
   * @param data input data of type T
   * @param indices indices of the elements of data to copy. If an index appears
   *   more than once, the element is copied several times. Indices that are
   *   out of the bounds of data are ignored. TN must be some kind of integer
   * @param ret (out) elements copied from data of the given indices, in the
   *   same order than the indices
   */
  template<class T, class TN>
  static void copyIndices(const std::vector<T> &data,
    const std::vector<TN> &indices, std::vector<T> &ret);
...
template<class T, class TN>
void STL::copyIndices(const std::vector<T> &data,
  const std::vector<TN> &indices, std::vector<T> &ret)
{
  ret.clear();
  ret.reserve(indices.size());

  typename std::vector<TN>::const_iterator iit;
  for(iit = indices.begin(); iit != indices.end(); ++iit)
  {
    if(*iit >= 0 && *iit < data.size()) ret.push_back(data[*iit]);
  }
}
