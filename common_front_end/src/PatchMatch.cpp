...
*/
  ...
  else if( fBestScore > g_common_cvars.match_error_threshold) {
    eFlag  = NoMatchOnLine;
  }
  else if( fBestScore*g_common_cvars.match_error_factor >= fSecondBest ) {
    eFlag  = NoMatchOnLine;
  }
  else{
    eFlag = GoodMatch;
    ...
    )
{
  ...
  } else if (best_score*g_common_cvars.match_error_factor >= second_best_score
             && second_feat
